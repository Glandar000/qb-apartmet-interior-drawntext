...
local function RegisterApartmentEntranceZone(apartmentID, apartmentData)
    ...
    zone:onPlayerInOut(function (isPointInside)
        local shouldRun = isPointInside
        if isPointInside and not InApartment then
            --[[exports['qb-drawtextui']:PopupText({header = 'Press [E] to enter your', text = 'Apartment'})
            CreateThread(function()
                while shouldRun do
                    if isPointInside and IsControlJustPressed(0, 38) then
                        TriggerEvent('apartments:client:EnterApartment')
                        shouldRun = isPointInside
                    end
                    Wait(5)
                end
            end)--]]
        else
            exports['qb-drawtextui']:ClosePopup()
            shouldRun = isPointInside
        end
    end)
    ...
    zone:onPlayerInOut(function (isPointInside)
        if isPointInside and text then
            if targetKey == "entrancePos" then
                exports['qb-drawtextui']:PopupText({header = 'Press [E] to leave your', text = 'Apartment'})
            else
                exports['qb-drawtextui']:PopupText({header = 'Press [E] to open', text = text})
            end
        else
            if targetKey == "entrancePos" then
                IsInsideLeaveZone = isPointInside
                exports['qb-drawtextui']:ClosePopup()
            else
                exports['qb-drawtextui']:ClosePopup()
            end
...
local function SetInApartmentTargets()
    local entrancePos = vector3(-258.5788, -955.4553, 2.3918092)
    local stashPos = vector3(-257.9261, -959.6642, 2.3918094)
    local outfitsPos = vector3(-260.6549, -963.3195, 2.3918094)

    RegisterInApartmentZone('stashPos', stashPos, 0, Lang:t('text.open_stash'))
    RegisterInApartmentZone('outfitsPos', outfitsPos, 0, Lang:t('text.change_outfit'))
    ...
    QBCore.Functions.TriggerCallback('apartments:GetApartmentOffset', function(offset)
        if offset == nil or offset == 0 then
            QBCore.Functions.TriggerCallback('apartments:GetApartmentOffsetNewOffset', function(newoffset)
                ...
                local data = exports['qb-interior']:CreateFurniMotelModern(coords)
                Wait(100)
                HouseObj = data[1]
                POIOffsets = data[2]
                InApartment = true
                ...
                Wait(100)
                TriggerEvent('qb-backitems:client:refreshObjects')
...
RegisterNetEvent('apartments:client:ChangeOutfit', function()
    ...
    TriggerServerEvent("qb-outfits:client:forceUI")
end)
...
CreateThread(function()
    exports['qb-drawtextui']:Popup({
        coords = vector3(-263.6527, -959.7, 31.223133),
        trigger = 'apartments:client:EnterApartment',
        triggerType = 'client',
        header = 'Press [E] to enter your',
        text = 'Apartment',
        radius = 1.5,
    })
end)
